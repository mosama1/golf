...
<div class="reservas-fecha">
  ...
  <div class="icon">
    <center>
      ...
      <input type="date" min="<?php echo date("Y-m-d") ?>" name="date" id="dateReserva" value="<?php echo date("Y-m-d", strtotime($_POST['date'])) ?>" >
    </center>
  </div>
  ...
  <div class="fecha">
    <p>
      <?php
        ...
        echo $dias[date('w', strtotime($_POST['date']))]." ".date('d', strtotime($_POST['date']))." de ".$meses[date('n', strtotime($_POST['date']))-1]. " del ".date('Y', strtotime($_POST['date'])) ;
      ...
      <tr>
        ...
        ?>
          <th id='th_<?php echo date("hi", strtotime($hora->horaInicio)+(3600*$n)) ?>'>
            <a href='#' onclick="buscarCatergoriaFecha(<?php echo $user->id ?>,<?php echo $_POST['id'] ?>,'<?php echo $_POST['tipo'] ?>','<?php echo date("Y-m-d", strtotime($_POST['date'])) ?>','<?php echo date("h:i a", strtotime($hora->horaInicio)+(3600*$n)) ?>', 'th_<?php echo date("hi", strtotime($hora->horaInicio)+(3600*$n)) ?>')" >
...
function buscarCatergoriaFecha(idUser, id, tipo, date, hora, id_th){
  function activar() {
  ...
  if (tipo == 'cancha') {
      $.ajax({
          type: "POST",
          url: "buscarCatergoriaFecha.php",
          data: {idUser:idUser, id:id, date:date, hora:hora},
    ...
    $.ajax({
      type: "POST",
      url: "buscarCatergoriaClaseFecha.php",
      data: {idUser:idUser, id:id, date:date, hora:hora},
      success: function(respuesta) {
